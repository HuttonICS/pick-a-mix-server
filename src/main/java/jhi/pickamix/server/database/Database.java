package jhi.pickamix.server.database;
...
import static jhi.pickamix.server.database.codegen.tables.Trials.*;
	...
	public static void init(String databaseServer, String databaseName, String databasePort, String username, String password, boolean initAndUpdate)
	{
		...
		if (initAndUpdate)
		{
			...
			if (!databaseExists)
			{
				...
				try
				{
					URL url = Database.class.getClassLoader().getResource("jhi/pickamix/server/util/database/init/db_setup.sql");
			...
			try
			{
				URL url = Database.class.getClassLoader().getResource("jhi/pickamix/server/util/database/init/views_procedures.sql");
	...
	public static DSLContext getContext(Connection connection)
	{
		Settings settings = new Settings()
			.withRenderMapping(new RenderMapping()
				.withSchemata(
					new MappedSchema().withInput(PickamixDb.PICKAMIX_DB.getQualifiedName().first())
